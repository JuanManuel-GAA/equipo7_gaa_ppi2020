...
router.get("/parque_calificado", (req, res) => {
  mysqlConnection.query("SELECT * FROM parque_calificado", (err, rows, fields) => {
    if (!err) {
...
router.post("/parque_calificado", (req, res) => {
  ...
  mysqlConnection.query(nuevoParque_calificado, [
    ID_parque,
    ID_usuario,
    calificacion,
    comentario,
...
router.put("/parque_calificado/:ID", (req, res) => {
  ...
  WHERE ID = ?`;
  mysqlConnection.query( actualizarParque_calificado, 
    [
    ID_parque,
    ID_usuario,
    calificacion,
